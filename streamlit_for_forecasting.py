...
!pip install streamlit pandas scikit-learn prophet statsmodels matplotlib
...
if 'df' in locals():
    ...
    forecast_methods = st.multiselect("Select forecasting methods", ["Prophet", "Simple Moving Average", "Exponential Smoothing"])

# Predictions
def forecast_prophet(data, periods):
    model = Prophet()
    model.fit(data.reset_index().rename(columns={'ds': 'ds', 'y': 'y'}))
...
if 'df' in locals() and 'forecast_methods' in locals() and forecast_methods:
    ...
    if "Prophet" in forecast_methods:
        try:
            prophet_predictions = forecast_prophet(train.copy(), periods_to_forecast)
            predictions['Prophet'] = prophet_predictions
            ax.plot(prophet_predictions.index, prophet_predictions['yhat'], label='Prophet Forecast')
        except Exception as e:
            st.error(f"Error with Prophet: {e}")
    ...
    if "Exponential Smoothing" in forecast_methods:
        try:
            ...
            predictions['Exponential Smoothing'] = es_predictions
...
if 'predictions' in locals() and predictions and not test.empty:
    ...
    for method, preds in predictions.items():
        if method == 'Prophet':
            y_true = test['y']
            y_pred = preds['yhat'][-len(test):].values
